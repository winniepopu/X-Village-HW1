...
class Matrix:
    def __init__(self, nrows, ncols):   # 初始建立 n*m 矩陣，各value為亂數值
        ...
        for i in range(self.row):
            ...
            self.matr.append(new_row)   # 將new_row list 插入 matr list，重複row次
    ...
    def mul(self,B):                                        # A * B
        ...
        if(self.col==B.row):                                # EX: for j=3的矩陣
            ...
            print("none")       # matrix A,B 無法相乘時，印出none
    ...
    def transpose(self):
        ...
        if self.matr!=[]:       # 當該矩陣不等於空矩陣時(matrix A,B 可相乘時)，執行transpose
            ...
            print("none")       # 當該矩陣等於空矩陣時(matrix A,B 無法相乘時)，印出none
...
A.add(B)                    # A+B
A.sub(B)                    # A-B
A.mul(B).transpose()        # A*B & transpose of A*B
